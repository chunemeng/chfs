...
auto FileOperation::read_as_regular_file(inode_id_t id, std::vector<u8> &content) -> ChfsNullResult {
    ...
    content.resize(size * sizeof(block_id_t));
    memcpy(content.data(), &inode_p->blocks[0], size * sizeof(block_id_t));
    return KNullOk;
}
auto FileOperation::append_regular_file(inode_id_t id, block_id_t block_id, uint32_t mac_id) -> ChfsNullResult {
    ...
    auto size = inode_p->inner_attr.size * 2;
    if (size + 4 > inode_p->nblocks) {
        return {ErrorType::OUT_OF_RESOURCE};
    }

    inode_p->blocks[size] = block_id;
    inode_p->blocks[size + 1] = mac_id;
    inode_p->inner_attr.size += 1;
