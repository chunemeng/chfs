...
private:
    ...
    bool is_election_time_out() {
        return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - last_heartbeat).count() > election_timeout;
    }
...
void RaftNode<StateMachine, Command>::send_request_vote(int target_id, RequestVoteArgs arg) {
    ...
    if (rpc_clients_map[target_id] == nullptr || rpc_clients_map[target_id]->get_connection_state() != rpc::client::connection_state::connected) {
        bool rpc_client_connected = rpc_clients_map[target_id] == nullptr;
...
void RaftNode<StateMachine, Command>::run_background_election() {
    ...
    election_timeout = HEARTBEAT_INTERVAL + (generator() % HEARTBEAT_INTERVAL) * 1.5;
        ...
        }
        {
            ...
            if (role == RaftRole::Follower) {
                ...
                if (is_election_time_out()) {
                    auto gen = generator();
                    election_timeout = HEARTBEAT_INTERVAL + (gen % HEARTBEAT_INTERVAL) * 1.5;
            ...
            if (role == RaftRole::Candidate && is_election_time_out()) {
                ...
                for (const auto &client: rpc_clients_map) {
                    ...
                    thread_pool->enqueue([this, target_id = client.first]() {
                        send_request_vote(target_id, RequestVoteArgs{current_term, my_id, log_storage->last_log_index_, log_storage->last_log_term_});
                    });
                }
            }
        }
...
void RaftNode<StateMachine, Command>::run_background_apply() {
    ...
    while (true) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(APPLY_INTERVAL));
    }
}
