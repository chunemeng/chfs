...
auto InodeManager::allocate_inode(InodeType type, block_id_t bid, std::vector<std::shared_ptr<BlockOperation>> &ops, bool &is_err) -> ChfsResult<inode_id_t> {
    ...
    for (auto iter = iter_res.unwrap(); iter.has_next();
        ...
        if (free_idx) {
            ...
            ops.emplace_back(std::make_shared<BlockOperation>(bid, buffer));
            ...
            res = set_table(inode_id, bid, ops, is_err);
...
auto InodeManager::set_table(inode_id_t idx, block_id_t bid, std::vector<std::shared_ptr<BlockOperation>> &ops, bool &is_err) -> ChfsNullResult {
    ...
    *(reinterpret_cast<block_id_t *>(buffer.data() + block_offset)) = bid;
    ops.emplace_back(std::make_shared<BlockOperation>(table_block_idx + 1, buffer));
